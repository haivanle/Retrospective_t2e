...
def load_sprint_dataset(location, outcome, intervention='INTENSIVE'):
  ...
  features = {'SPRINT-POP/data/baseline.sas7bdat': all_feats,
              'SPRINT/data/bl_meds_physexam.sas7bdat': ['WEIGHT', 'HEIGHT']}

  outcomedt = 'T_'+outcome
  outcome = 'EVENT_'+outcome

  outcomes, features = _load_generic_biolincc_dataset(outcome_tbl='SPRINT-POP/data/outcomes.sas7bdat', 
                                                      time_col=outcomedt, 
                                                      event_col=outcome,
                                                      features=features, 
                                                      id_col='MASKID',
                                                      location=location+'SPRINT_2021a/')
  
  cat_feat = [cat_feat for cat_feat in cat_feat if cat_feat is not intervention]
  num_feat = num_feat + ['WEIGHT', 'HEIGHT']
  all_feats = list(set(cat_feat + num_feat))
  ...
  # Convert Weight and Height to Standard Metric Unit Measurement
  features.WEIGHT = features.WEIGHT * 0.45
  features.HEIGHT = features.HEIGHT * 2.54
...
def load_aimhigh_dataset(location, outcome):
  ...
  intervention = features['RandAssi'].dropna()
  features = features[all_feats]
  ...
  features = features.rename(columns={"Age65": "Baseline_>65_years_old", 
                                      ...
                                      "Age": "Baseline_Age",
                                      "BMI": "Baseline_BMI",
                                      "PE_Ht": "Baseline_Height_CM",
                                      "PE_Wt": "Baseline_Weight_KG"})
...
def load_bari_2d_dataset(location, intervention='cardtrt', random_seed=0, **kwargs):
  ...
  num_feat = ['weight', 'sbp_stand', 'dbp_stand', 'screat', 
              ...
              'hdl','ldl','trig', 'acr', 'age', 'bmi', 'height']
...
def load_accord(endpoint=None, features=None, location=''):  
  ...
  if features is None:
    ...
    features = {
                ...
                'ACCORD/4-Data Sets - CRFs/4a-CRF Data Sets/f07_baselinehistoryphysicalexam.sas7bdat': ['ht_cm', 'wt_kg']
                }
...
def load_peace_dataset(location, outcome, intervention='TX'):
...
def load_allhat_antihypertensive_dataset(location, outcome, intervention = 'RZGROUP'):
  ...
  cat_feat = [cat_feat for cat_feat in cat_feat if cat_feat is not intervention]
  all_feats = list(set(cat_feat + num_feat)) + ['ACREAT']
  ...
  # Convert Weight and Height to Standard Metric Unit Measurement
  features.BLWGT = features.BLWGT * 0.45
  features.BLHGT = features.BLHGT * 2.54
  ...
  features = features.rename(columns={"ETHNIC": "Ethnicity",
                                      ...
                                      'BLWGT': "Baseline_Weight_KG", 
                                      'BLHGT': "Baseline_Height_CM",
                                      ...
                                      "BLBMI": "Baseline_BMI",
                                      "ACREAT": "Baseline_Serum_creatinine"})
...
def load_mesa_dataset(location, outcome):
  
  """Helper function to load and preprocess the MESA dataset.

  The MESA Dataset is a subset of 6,800 ethnically diverse participants of the
  Multi-Ethnic Study of Atherosclerosis Trial [1] for studying the prevalence, 
  correlates, and progression of subclinical CVD and risk factors that 
  predict progression to clinically overt CVD, and that 
  predict progression of subclinical disease itself. 

  References
  ----------
  [1] D. E. Bild, D. A. Bluemke, G. L. Burke, R. Detrano, A. V. Diez Roux, A. R. Folsom, 
  P. Greenland, D. R. JacobsJr, R. Kronmal, K. Liu, et al., “Multi-ethnic study of 
  atherosclerosis: objectives and design,” American journal of epidemiology, vol. 156, 
  no. 9, pp. 871–881, 2002. https://pubmed.ncbi.nlm.nih.gov/12397006/ 

  Website and Documentation
  ----------
  [1] https://biolincc.nhlbi.nih.gov/studies/mesa/
  [2] https://biolincc.nhlbi.nih.gov/media/studies/mesa/data_dictionary/MESA_v2022a.pdf
  ...
  Outcomes Description:
    ...
    ang - Angina Pectoris
    ptca - PTCA, Cor. Stent, or Cor. Atherectomy
    cbg - Coronary Bypass Graft
    otherev - Other Revascularization
    chf - Congestive Heart Failure 
    pvd - Peripheral Vascular Disease
    strk - Stroke
    tia- Transient Ischemic Attack
    chdh - Coronary Heart Disease Hard
    chda - Coronary Heart Disease All
    chdh - Cardiovascular Disease Hard
    cvda - Cardiovascular Disease All
    revc - Coronary Revascularization
  """

  cat_feat = ['gender1', 'diabet1', 'ecglvh1c', 'aspirin1', 'estrgn1c', 
              'lipid1c', 'race1c']

  num_feat = ['ldl1', 'hdl1', 'htcm1', 'wtlb1', 'age1c', 'trig1', 'creatin1',
              'bmi1c', 'sbpcp1', 'dbpcp1', 'chol1', 'glucos1c', 'cepgfr1c']

  all_feats = list(set(cat_feat + num_feat))

  features = {'Exam1/Data/mesae1dres06192012.sas7bdat': all_feats}

  outcomedt = outcome+'tt'

  outcomes, features = _load_generic_biolincc_dataset(outcome_tbl='Events/CVD/Data/mesaevthr2015_drepos_20200330.sas7bdat', 
                                                      time_col=outcomedt, 
                                                      event_col=outcome,
                                                      features=features, 
                                                      id_col='MESAID',
                                                      location=location+'MESA/PRIMARY/')

# Convert Weight to Standard Metric Unit Measurement
  features.wtlb1 = features.wtlb1 * 0.45
  ...
  features = features.rename(columns={'gender1': 'Sex',
                                    'diabet1': 'Baseline_History_Diabetes',
                                    'ecglvh1c': 'Baseline_History_Left_Ventricular_Hypertrophy_Echocardiogram',
                                    'aspirin1': 'Baseline_History_Aspirin',
                                    'estrgn1c': 'Baseline_Estrogen_Supplementation',
                                    'lipid1c': 'Baseline_Lipid_Lowering_Therapy',
                                    'race1c': 'Race',
                                    'chol1': 'Baseline_Total_Cholesterol',
                                    'ldl1': 'Baseline_LDL_Cholesterol',
                                    'hdl1': 'Baseline_HDL_Cholesterol',
                                    'htcm1': 'Baseline_Height_CM',
                                    'wtlb1': 'Baseline_Weight_KG',
                                    'bmi1c': 'Baseline_BMI',
                                    'sbpcp1': 'Baseline_Seated_Systolic_Blood_Pressure',
                                    'dbpcp1': 'Baseline_Seated_Diastolic_Blood_Pressure',
                                    'glucos1c': 'Baseline_Glucose',
                                    'cepgfr1c': 'Baseline_Estimated_Glomerular_Filtration_Rate',
                                    'age1c': 'Baseline_Age',
                                    'trig1': 'Baseline_Triglycerides',
                                    'creatin1': 'Baseline_Serum_creatinine'})

  features['Sex'].replace({1.0: "Male", 0.0: "Female"}, inplace=True)
  features['Baseline_History_Diabetes'].replace({1.0: "Yes", 0.0: "No", 9.0: "Don't know"}, inplace=True)
  features['Baseline_History_Left_Ventricular_Hypertrophy_Echocardiogram'].replace({1.0: "Yes", 0.0: "No"}, inplace=True)
  features['Baseline_History_Aspirin'].replace({1.0: "Yes", 0.0: "No", 9.0: "Don't know"}, inplace=True)
  features['Baseline_Estrogen_Supplementation'].replace({1.0: "Yes", 0.0: "No"}, inplace=True)
  features['Baseline_Lipid_Lowering_Therapy'].replace({1.0: "Yes", 0.0: "No"}, inplace=True)
  features['Race'].replace({1.0: "CAUCASIAN", 2.0: "CHINESE", 3.0: "AFRICAN AMERICAN", 4.0: "HISPANIC"}, inplace=True)

  cat_feats = ["Baseline_History_Diabetes", 
               "Baseline_History_Left_Ventricular_Hypertrophy_Echocardiogram", 
               "Sex",
               "Baseline_History_Aspirin",
               "Baseline_Estrogen_Supplementation",
  ...
  return outcomes, features, cat_feats, num_feats


def load_aric_dataset(location, outcome, intervention='INTENSIVE'):
  
  """Helper function to load and preprocess the MESA dataset.

  The ARIC Dataset is a subset of 15,792 ethnically diverse participants of the
  Atherosclerosis Risk in Communities Study [1] for studying the associations of 
  factors with prevalence of atherosclerosis and incidence of CHD, clinical stroke 
  and other cardiovascular diseases and measure cardiovascular disease occurrence 
  and trends and relate these to community levels of, and changes in, risk factors, 
  ...
  Website and Documentation
  ----------
  [1] https://biolincc.nhlbi.nih.gov/studies/aric/
  [2] https://biolincc.nhlbi.nih.gov/media/studies/aric/data_dictionary/ARIC_v2021a.pdf
  """

  cat_feat = ['GENDER', 'CLVH01', 'HXOFMI02', 'RACEGRP', 'CIGT01',
              'DIABTS02']
  
  num_feat = ['V1AGE01', 'BMI01', 'GLUCOS01', 'TCHSIU01', 'TRGSIU01',
              'HDL01', 'LDL02']

  all_feats = list(set(cat_feat + num_feat))

  features = {'derive13.sas7bdat': all_feats}


def load_cardia_dataset(location, outcome):
  
  """Helper function to load and preprocess the MESA dataset.

  The CARDIA Dataset is a subset of 5,115 ethnically diverse participants of the
  Coronary Artery Risk Development in Young Adults Study [1] to understand 
  determinants of left ventricular mass, emerging obesity and hypertension, 
  and sequelae of hypertension in pregnancy.
  ...
  Website and Documentation
  ----------
  [1] https://biolincc.nhlbi.nih.gov/studies/cardia/
  [2] https://biolincc.nhlbi.nih.gov/media/studies/cardia/data_dictionary/CARDIA_2022a.pdf
  
  Outcomes Description:
    DEAD - Death
    MI - Myocardial Infarction 
    CHFFNF - Congestive Heart Failure Fatal or Non-fatal
    CHF - Congestive Heart Failure Non-fatal
    STROKEA - Stroke Any
    STROKEH - Stroke Hard
    STROKEAFNF - Any Stroke Fatal or Non-fatal 
    STROKEHFNF - Hard Stroke Fatal or Non-fatal 
    TIA - Transient Ischemic Attack
    CHDH - Coronary Heart Disease Hard
    CHDA - Coronary Heart Disease Any
    CVDH - Cardiovascular Disease Hard
    CVDA - Cardiovascular Disease Any
    CVDAFNF - Any Cardiovascular Disease Fatal or Non-fatal
    CVDHFNF - Hard Cardiovascular Disease Fatal or Non-fatal
    CHDAFNF - Any Coronary Heart Disease Fatal or Non-fatal
    CHDHFNF - Hard Coronary Heart Disease Fatal or Non-fatal 
    revc - Coronary Revascularization
    CRV - Cardiac Revascularization Non-fatal  
  """

  features = {'Y00/DATA/aaf01.sas7bdat': ['A01SEX', 'A01RACE2', 'A01SMNOW', 'A01AGE2'],
              'Y00/DATA/aaf02.sas7bdat': ['A02SBP', 'A02DBP'],
              'Y00/DATA/aalip.sas7bdat': ['AL1CHOL', 'AL1NTRIG', 'AL1HDL', 'AL1LDL'],
              'Y00/DATA/aachem.sas7bdat': ['AL3_GLU', 'AL3CREAT'],
              'Y00/DATA/aaf20.sas7bdat': ['A20HGT', 'A20WGT', 'A20BMI'],
              'Y00/DATA/aaf08v2.sas7bdat': ['A08DIAB']}

  if outcome == 'DEATH': outcome = 'DEAD' 
  else: outcome = outcome 

  if outcome == 'DEAD': outcomedt = 'DEATHATT'
  else: outcomedt = outcome+'ATT'

  outcomes, features = _load_generic_biolincc_dataset(outcome_tbl='OUTCOMES/DATA/outcomes2021.sas7bdat', 
                                                      time_col=outcomedt, 
                                                      event_col=outcome,
                                                      features=features, 
                                                      id_col='PID',
                                                      location=location+'CARDIA/Main_Study/')

# Convert Weight to Standard Metric Unit Measurement
  features.A20WGT = features.A20WGT * 0.45
  ...
  features = features.rename(columns={'A01SEX': 'Sex',
                                    'A08DIAB': 'Baseline_History_Diabetes',
                                    'A01SMNOW': 'Baseline_Smoke_Status',
                                    'A01RACE2': 'Race',
                                    'AL1CHOL': 'Baseline_Total_Cholesterol',
                                    'AL1LDL': 'Baseline_LDL_Cholesterol',
                                    'AL1HDL': 'Baseline_HDL_Cholesterol',
                                    'A20HGT': 'Baseline_Height_CM',
                                    'A20WGT': 'Baseline_Weight_KG',
                                    'A20BMI': 'Baseline_BMI',
                                    'A02SBP': 'Baseline_Seated_Systolic_Blood_Pressure',
                                    'A02DBP': 'Baseline_Seated_Diastolic_Blood_Pressure',
                                    'AL3_GLU': 'Baseline_Glucose',
                                    'A01AGE2': 'Baseline_Age',
                                    'AL1NTRIG': 'Baseline_Triglycerides',
                                    'AL3CREAT': 'Baseline_Serum_creatinine'})

  features['Sex'].replace({1.0: "Male", 2.0: "Female"}, inplace=True)
  features['Baseline_History_Diabetes'].replace({1.0: "No", 2.0: "Yes", 8.0: "Not sure"}, inplace=True)
  features['Baseline_Smoke_Status'].replace({1.0: "No", 2.0: "Yes"}, inplace=True)
  # 0.0, 1.0, 2.0, 4.0 
  features['Race'].replace({3.0: "Hispanic", 4.0: "Black", 5.0: "White", 6.0: "American Indian or Alaskan Native", 
                            7.0: "Asian or Pacific Islander", 8.0 : "Don't Know", 9.0: "No Answer"}, inplace=True)
...
def load_best_dataset(location, outcome, intervention = 'GROUP'):

  """Helper function to load and preprocess the BEST dataset.

  The Beta-Blocker Evaluation in Survival Trial (BEST) Dataset is a subset of 
  2,708 patients with heat failure and a left ventricular ejection fraction of 
  35 percent or lower randomly assigned to double-blind treatment with either 
  bucindolol (1,354 patients) or placebo (1,354 patients) and followed for 
  the primary end point of death from any cause.[1]
  ...
  Website and Documentation
  ----------
  [1] https://biolincc.nhlbi.nih.gov/studies/best/
  [2] https://biolincc.nhlbi.nih.gov/media/studies/best/data_dictionary/BEST.pdf
  
  """

  features = {'pe.sas7bdat': ['PEHR', 'PEBPS', 'PEBPD', 'PEWT', 'PEHT'],
              't.sas7bdat': ['GROUP'],
              'br.sas7bdat': ['BRNYHA34', 'BREFLT35', 'BRMI6', 'BRANG', 'BRRACE', 'BRSEX']}

  if outcome == 'MOCAUSE': outcomedt = 'MODAYS'

  outcomes, features = _load_generic_biolincc_dataset(outcome_tbl='mort_1.sas7bdat', 
                                                      time_col=outcomedt, 
                                                      event_col=outcome,
                                                      features=features, 
                                                      id_col='BEST_ID',
                                                      location=location+'BEST/Sas_Data/')

# Convert Weight and Height to Standard Metric Unit Measurement
  features.PEWT = features.PEWT * 0.45
  features.PEHT = features.PEHT * 2.54
  ...
  intervention[intervention == 2.0] = 'Bucindolol'
  intervention[intervention == 1.0] = 'Placebo'

  features = features.rename(columns={'PEHR': 'Baseline_Heart_Rate',
                                    'PEHT': 'Baseline_Height_CM',
                                    'PEWT': 'Baseline_Weight_KG',
                                    'PEBPS': 'Baseline_Seated_Systolic_Blood_Pressure',
                                    'PEBPD': 'Baseline_Seated_Diastolic_Blood_Pressure',
                                    'BRNYHA34': 'Baseline_NYHA_Class_III_IV',
                                    'BREFLT35': 'Baseline_Ejection_Fraction_35%',
                                    'BRMI6': 'Baseline_History_Myocardial_Infarction',
                                    'BRANG': 'Baseline_Severe_Unstable_Angina',
                                    'BRRACE': 'Race',
                                    'BRSEX': 'Sex'})

  features['Sex'].replace({1.0: "Male", 2.0: "Female"}, inplace=True)
  features['Baseline_Severe_Unstable_Angina'].replace({1.0: "Yes", 2.0: "No"}, inplace=True)
  features['Baseline_History_Myocardial_Infarction'].replace({1.0: "Yes", 2.0: "No"}, inplace=True)
  features['Baseline_Ejection_Fraction_35%'].replace({1.0: "Yes", 2.0: "No"}, inplace=True)
  features['Baseline_NYHA_Class_III_IV'].replace({1.0: "Yes", 2.0: "No"}, inplace=True)
  features['Race'].replace({1.0: "Black", 2.0: "Other"}, inplace=True)

  cat_feats = ["Baseline_NYHA_Class_III_IV", 
               "Baseline_Ejection_Fraction_35%",
               "Race",
               "Baseline_History_Myocardial_Infarction",
               "Baseline_Severe_Unstable_Angina",
               "Sex"]
